...
public class StudentiController implements Initializable {
	...
	@FXML
	private TableColumn<Student, String> col_dt;

	ObservableList<Student> list = FXCollections.observableArrayList(ucitajStudenta());
	...
	private List<Student> ucitajStudenta() {
	...
	private void search(ActionEvent event) {
		...
		filter.setPredicate((Predicate<? super Student>) (Student stud) -> {
			if (searchJmbag.getText().isEmpty() || searchJmbag.getText() == null) {
				return true;
			} else if (stud.getJmbag().contains(searchJmbag.getText())) {
				return true;
			}
			return false;
		});
		...
		filter2.setPredicate((Predicate<? super Student>) (Student stud) -> {
			if (searchPrezime.getText().isEmpty() || searchPrezime.getText() == null) {
				return true;
			} else if (stud.getPrezime().contains(searchPrezime.getText())) {
				return true;
			}
			return false;
		});
		...
		filter3.setPredicate((Predicate<? super Student>) (Student stud) -> {
			if (searchIme.getText().isEmpty() || searchIme.getText() == null) {
				return true;
			} else if (stud.getIme().contains(searchIme.getText())) {
				return true;
			}
			return false;
		});
		...
		filter4.setPredicate((Predicate<? super Student>) (Student stud) -> {

			if (searchDatum.getValue() == null) {
				return true;
			} else if (stud.getDatumRodjenja().equals(searchDatum.getValue())) {
				return true;
			}
			return false;
		});
